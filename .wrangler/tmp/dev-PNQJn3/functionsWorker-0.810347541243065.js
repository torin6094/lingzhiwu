...
// .wrangler/tmp/bundle-TAM5gM/checked-fetch.js
...
// .wrangler/tmp/pages-cFDG2t/functionsWorker-0.810347541243065.mjs
...
var JIANSHU_USER_ID = "9aa9f349f3fb";
...
__name(fetchJianshuArticles, "fetchJianshuArticles");
__name2(fetchJianshuArticles, "fetchJianshuArticles");
...
function formatNoteDate(note) {
  const timestamp = note.first_shared_at || note.created_at || note.published_at || note.first_shared_time || note.share_time || note.mounted_at;
  if (timestamp) {
...
__name(formatNoteDate, "formatNoteDate");
__name2(formatNoteDate, "formatNoteDate");
...
__name2(fetchFromApi, "fetchFromApi");
...
__name(fetchFromHtml, "fetchFromHtml");
__name2(fetchFromHtml, "fetchFromHtml");
function parseHtmlArticles(html) {
  ...
  for (const item of items.slice(0, 10)) {
    ...
    const timeMatch = item.match(/<span[^>]*class="time"[^>]*data-shared-at="([^"]+)"/) || item.match(/<span[^>]*class="time"[^>]*>([\s\S]*?)<\/span>/);
    let image = "";
    const imgMatch = item.match(/<img[^>]*src="([^"]+)"[^>]*>/) || item.match(/<img[^>]*data-src="([^"]+)"[^>]*>/) || item.match(/<img[^>]*data-original-src="([^"]+)"[^>]*>/);
    if (imgMatch) {
      image = imgMatch[1];
...
__name(parseHtmlArticles, "parseHtmlArticles");
__name2(parseHtmlArticles, "parseHtmlArticles");
...
async function onRequest2(context) {
  ...
  const location = url.searchParams.get("location") || "101020100";
...
// .wrangler/tmp/bundle-TAM5gM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TAM5gM/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.810347541243065.js.map
